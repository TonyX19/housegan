...
fp_dataset = FloorplanGraphDataset(rooms_path, transforms.Normalize(mean=[0.5], std=[0.5]), split='all',target_set=opt.target_set)
...
for i, batch in enumerate(fp_iter):#len(fp_iter) = all_samples/batch_size
    ...
    all_data.append([np.array(mks),np.array(nds)])
